...
export async function fetchUserProfile(userId: string): Promise<ServiceResult<UserRow | null>> {
  try {
    ...
    return failure(
      AppError.fromNetworkError(error, { userId, operation: 'fetchUserProfile' })
    );
  }
}
 ...
 * 사용자 생성 (신규 가입 시)
 */
export async function createUser(userId: string, email?: string | null): Promise<ServiceResult<UserRow>> {
  try {
    const { data, error } = await supabase
      .from('users')
      .insert({
    ...
    return failure(
      AppError.fromNetworkError(error, { userId, operation: 'createUser' })
    );
  }
}
...
export async function completeOnboarding(userId: string): Promise<ServiceResult<UserRow>> {
  try {
    ...
    if (error) {
      ...
      return failure(AppError.fromSupabaseError(error, { userId, operation: 'completeOnboarding' }));
    }
    ...
    return failure(
      AppError.fromNetworkError(error, { userId, operation: 'completeOnboarding' })
    );
  }
}
...
export async function deleteUser(userId: string, reason?: string): Promise<ServiceResult<void>> {
  try {
    ...
    return failure(
      AppError.fromNetworkError(error, { userId, operation: 'deleteUser' })
    );
  }
}
