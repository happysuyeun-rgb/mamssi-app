...
export default function SocialLoginButtons({ mode, disabled = false }: SocialLoginButtonsProps) {
  const handleGoogleLogin = async () => {
    ...
    if (!hasValidSupabaseConfig()) {
      const errorMsg = '환경변수가 없어 소셜 로그인을 사용할 수 없습니다. VITE_SUPABASE_URL과 VITE_SUPABASE_ANON_KEY를 .env 파일에 설정해주세요.';
    ...
    try {
      ...
      if (error) {
        ...
        notify.error(`구글 로그인에 실패했어요. ${error.message || '잠시 후 다시 시도해 주세요.'}`, '❌');
        return;
      }
